...
class LoggerUIImpl(val title: String)(implicit fsc: FSContext) extends LoggerUI:
  ...
  def log(mesg: String): Unit = fsc.sendToPage(
    JS.prepend2(loggerOutputId, <pre style="white-space: pre-wrap; margin: 0;">{mesg}</pre>)
  )

  def info(mesg: String): Unit = fsc.sendToPage(
    JS.prepend2(
      loggerOutputId,
      <pre style="white-space: pre-wrap; margin: 0;" class="text-info">{mesg}</pre>,
    )
  )

  def success(mesg: String): Unit = fsc.sendToPage(
    JS.prepend2(
      loggerOutputId,
      <pre style="white-space: pre-wrap; margin: 0;" class="text-success">{mesg}</pre>,
    )
  )

  def danger(mesg: String): Unit = fsc.sendToPage(
    JS.prepend2(
      loggerOutputId,
      <pre style="white-space: pre-wrap; margin: 0;" class="text-danger">{mesg}</pre>,
    )
  )

  def warn(mesg: String): Unit = fsc.sendToPage(
    JS.prepend2(
      loggerOutputId,
      <pre style="white-space: pre-wrap; margin: 0;" class="text-warning">{mesg}</pre>,
    )
  )

  def debug(mesg: String): Unit = fsc.sendToPage(
    JS.prepend2(
      loggerOutputId,
      <pre style="white-space: pre-wrap; margin: 0;" class="text-black-50">{mesg}</pre>,
    )
  )
  ...
  private val modal = new BSModal5Base:
    ...
    override def modalBodyContents()(implicit fsc: FSContext): NodeSeq =
      div
        .withId(loggerOutputId)
        .withStyle("min-height: 400px; background: #f5f5f5; max-height: 800px; overflow: auto;")
        .apply:
          ""

    override def modalFooterContents()(implicit fsc: FSContext): Option[NodeSeq] = Some:
      if hasFinished then BSBtn().BtnSecondary.lbl("Close").onclick(hideAndRemoveAndDeleteContext()).btn
      else
...
object LoggerUI:
  ...
  def runInSeparateThreadAndOpenProgressModal(
    title: String
  )(
    code: LoggerUI => Unit
