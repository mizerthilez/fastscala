...
class JSTreePage extends MultipleCodeExamples2Page():
  ...
  override def renderContentsWithSnippets()(implicit fsc: FSContext): Unit =
    renderSnippet("Source"):
      lazy val data = Source
        .fromResource("world-cities.csv")
        .getLines()
        .drop(1)
        .map(_.split(","))
        .collect:
      ...
      lazy val country2Region2City: Map[String, Map[String, Vector[String]]] =
        data
          .groupBy(_._1)
          .transform: (k, v) =>
            v.groupBy(_._2)
      ...
      val jsTree = new JSTree[Unit]:
        override val rootNodes: Seq[JSTreeNode[Unit]] =
          List:
            JSTreeSimpleNode[Unit]("Cities of the world", (), s"root"):
              country2Region2City.toVector
                .sortBy(_._1)
                .map:
                  case (country, region2City) =>
                    JSTreeSimpleNode[Unit](country, (), s"c_$country"):
                      region2City.toVector
                        .sortBy(_._1)
                        .map:
                          case (region, cities) =>
                            JSTreeSimpleNode[Unit](region, (), s"r_$region"):
