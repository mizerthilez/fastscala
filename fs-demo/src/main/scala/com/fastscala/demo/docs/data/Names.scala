...
object Names:
  val FirstNames: List[String] =
    Source.fromInputStream(getClass.getResourceAsStream("/first_names.txt")).getLines().toList
  val LastNames: List[String] =
    Source.fromInputStream(getClass.getResourceAsStream("/last_names.txt")).getLines().toList
