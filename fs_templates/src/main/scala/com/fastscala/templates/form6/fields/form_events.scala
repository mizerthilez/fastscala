...
case class ChangedField(field: F6Field)(implicit renderedWithHints: Seq[RenderHint])
    extends FormEvent

/** Requested submit by, for example, pressing return.
  */
case class RequestedSubmit(onField: F6Field)(implicit renderedWithHints: Seq[RenderHint])
    extends FormEvent
