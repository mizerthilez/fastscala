...
trait F6FieldWithLabel extends F6FieldInputFieldMixin:
  ...
  def label: Option[NodeSeq] = _label()
          ...
          processInputElem(<input type={inputType}
            ...
            fsc
              ...
              .cmd
                   ...
                   onkeypress={
            s"event = event || window.event; if ((event.keyCode ? event.keyCode : event.which) == 13) {${Js.evalIf(hints.contains(SaveOnEnterHint))(Js.blur(elemId) & form.onSaveClientSide())}}"
          }
...
class F6DateField(
  ...
  def toString(value: java.time.LocalDate): String =
    value.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"))

  def fromString(str: String): Either[String, java.time.LocalDate] = scala.util
    .Try(java.time.LocalDate.parse(str, DateTimeFormatter.ofPattern("yyyy-MM-dd")))
    .toEither
    .left
    .map(_ => "Invalid input")
...
class F6DateTimeOptField()(implicit renderer: TextF6FieldRenderer)
  ...
  def toString(value: Option[java.time.LocalDateTime]): String =
