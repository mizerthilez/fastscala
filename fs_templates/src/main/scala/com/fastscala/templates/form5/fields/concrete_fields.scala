...
class F5VerticalField(
  ...
  override def render()(implicit form: Form5, fsc: FSContext, hints: Seq[RenderHint]): Elem =
    ...
    else buildElem("div", "id" -> aroundId)(children.map(_.render())*)
...
object F5VerticalField:
  def apply(
    ...
    children: FormField*
  ) = new F5VerticalField(enabled, deps)(children*)
...
object F5HorizontalField:
  def apply[T <: FormField](
    ...
    children: (String, FormField)*
  ) = new F5HorizontalField(enabled, deps)(children*)
          ...
          <input type={inputType}
            ...
            fsc
              ...
              .cmd
                   ...
                   onkeypress={
            s"event = event || window.event; if ((event.keyCode ? event.keyCode : event.which) == 13) {${Js.blur(elemId) & Js.evalIf(hints.contains(SaveOnEnterHint))(form.onSaveClientSide())}}"
          }
...
class F5StringField(
  ...
  override def errors(): Seq[(ValidatableField, NodeSeq)] = super.errors() ++
    (if required() && currentValue.getOrElse("").trim == "" then
  ...
  additionalAttrs: Seq[(String, String)] = Nil,
      ...
      fromString = str =>
        Right(
          Some(str)
            .filter(_.trim != "")
            .map(str => LocalDate.parse(str, DateTimeFormat.forPattern("YYYY-MM-dd")))
        ),
      label = label,
      name = name,
      placeholder = placeholder,
...
) extends F5TextField[java.time.LocalDate](
      getOpt = () => get(),
      setOpt = optValue => set(optValue),
...
) extends F5TextField[java.time.LocalDateTime](
      getOpt = () => get(),
      setOpt = optValue => set(optValue),
...
class F5TimeOfDayField(
  ...
  additionalAttrs: Seq[(String, String)] = Nil,
      ...
      toString = _.map(value =>
        DateTimeFormat
          .forPattern("HH:mm")
          .print(new DateTime().withTime(value / 60, value % 60, 0, 0))
      ).map(_.trim).getOrElse(""),
      fromString = str =>
        if str.trim == "" then Right(None)
        else
...
class F5CheckboxField(
  ...
  val enabled: () => Boolean = () => true,
  val deps: Set[FormField] = Set(),
)(implicit renderer: CheckboxFieldRenderer
) extends StandardFormField
       with ValidatableField
       with StringSerializableField
       with FocusableFormField:
...
class F5SelectField[T](
  ...
  override def saveToString(): Option[String] =
    Some(toId(currentlySelectedValue, all().indexOf(currentlySelectedValue))).filter(_ != "0")
...
class F5CodeField(
  ...
  override def reRender()(implicit form: Form5, fsc: FSContext, hints: Seq[RenderHint]): Js =
    if !hints.contains(OnSaveRerender) then super.reRender()
    else Js.void
  ...
  override def render()(implicit form: Form5, fsc: FSContext, hints: Seq[RenderHint]): Elem =
    if !enabled() then buildElem("div", "style" -> "display:none;", "id" -> aroundId)()
      ...
      withFieldRenderHints { implicit hints =>
        ...
        val onChangeCallbackJS = fsc.callback(
...
class F5SaveButtonField[B](
  ...
  val btnRenderer = JS.rerenderableP[(B => B, Form5)](_ =>
    implicit fsc => {
...
class F5FileUploadField(
  ...
  def renderPreview(file: Option[(String, Array[Byte])])(implicit fsc: FSContext): Elem =
    buildElem("div")()
    ...
    if !enabled() then buildElem("div", "style" -> "display:none;", "id" -> aroundId)()
      ...
      withFieldRenderHints { implicit hints =>
        ...
        <form target={targetId} action={
          ...
          <iframe id={targetId} name={
          targetId
        } src="about:blank" onload="eval(this.contentWindow.document.body.innerText)" style="width:0;height:0;border:0px solid #fff;"><html><body></body></html></iframe>
          {
          label
            .map(label => <label for={elemId}>{label}</label>)
            .map(_.pipe(renderer.transforLabelElem(this)(_)).pipe(transforLabelElem))
          ...
          {
          <input name="file" type="file" id={inputId} onchange={Js.show(buttonId).cmd}/>
            .pipe(renderer.transforFileInputElem(this)(_))
            .pipe(transforFileInputElem)
        }
          {
          submitBtn
            .pipe(renderer.transforSubmitButtonElem(this))
            .pipe(transforSubmitButtonElem)
            .withId(buttonId)
          ...
          {
          resetBtn
            .map(renderer.transforResetButtonElem(this))
