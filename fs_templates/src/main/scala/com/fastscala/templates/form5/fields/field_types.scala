...
trait FormField:
  ...
  def onEvent(event: FormEvent)(implicit form: Form5, fsc: FSContext, hints: Seq[RenderHint]): Js =
    Js.void
...
trait StandardFormField extends FormField with ElemWithRandomId:
  ...
  def withFieldRenderHints[T](f: Seq[RenderHint] => T)(implicit renderHints: Seq[RenderHint]): T =
    f:
