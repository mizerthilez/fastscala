...
case class BarChartDataset(
  ...
  def encode(): Json = BarChartDataset.encode(this)

object BarChartDataset:
  given Encoder[ChartDatasetData] = Encoder.instance[ChartDatasetData](_.encode())
  given Encoder[BarChartDataset] = semiauto.deriveEncoder[BarChartDataset]

  def encode(bcd: BarChartDataset): Json = bcd.asJson

sealed trait ChartDatasetData:
  def encode(): Json
