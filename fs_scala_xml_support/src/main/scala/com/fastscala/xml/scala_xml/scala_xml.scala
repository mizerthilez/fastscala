...
object FSScalaXmlEnv extends FSXmlEnv:
  ...
  override def transformContents[E <: FSXmlEnv](using e: E)(elem: Elem, transform: NodeSeq => e.NodeSeq): Elem =
  ...
  given [E <: FSXmlEnv](using e: E): Conversion[scala.xml.Elem, e.Elem] =
    elem =>
      e.buildElem(
          elem.label,
          elem.attributes.toList.map(a => a.key -> a.value.map(_.toString()).mkString(" ")): _*
        )
        (elem.child.map(node => e.buildUnparsed(node.toString())): _*)
