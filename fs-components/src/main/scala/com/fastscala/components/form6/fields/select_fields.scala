...
abstract class F6SelectFieldBase[T]()(implicit renderer: SelectF6FieldRenderer)
...
class F6SelectOptField[T]()(implicit renderer: SelectF6FieldRenderer) extends F6SelectFieldBase[Option[T]]:
  ...
  override def errors(): Seq[(ValidatableF6Field, NodeSeq)] = super.errors() ++
    (if required && currentValue.isEmpty
     then Seq((this, buildText(renderer.defaultRequiredFieldLabel)))
     else Seq())

class F6SelectField[T](opts: () => Seq[T])(implicit renderer: SelectF6FieldRenderer)
    extends F6SelectFieldBase[T]
...
abstract class F6MultiSelectFieldBase[T]()(implicit renderer: MultiSelectF6FieldRenderer)
    extends StandardF6Field
       with F6FieldWithOptions[T]
       with F6FieldWithOptionIds[T]
       with ValidatableF6Field
            ...
            Js.selectedValues(Js.elementById(elemId)),
            {
              case ids =>
                ...
                form.onEvent(ChangedField(this)(hints)) &
                  (if hints.contains(ShowValidationsHint) || errorsAtRenderTime.nonEmpty || errors().nonEmpty
...
class F6MultiSelectField[T]()(implicit renderer: MultiSelectF6FieldRenderer)
    extends F6MultiSelectFieldBase[T]:
