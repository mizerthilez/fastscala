...
case class BSProgress(
...
) extends ClassEnrichableImmutable[BSProgress]
       with AttrEnrichableImmutable[BSProgress]:
  def setAttribute(name: String, value: String): BSProgress =
    copy(additionalAttrs = additionalAttrs :+ (name -> value))

  def addClass(clas: String): BSProgress = copy(progressClasses = progressClasses + " " + clas)
  ...
  def animated: BSProgress =
    copy(progressBarClasses = progressBarClasses + progress_bar_animated.getClassAttr)

  def bgPrimary: BSProgress = copy(progressBarClasses = progressBarClasses + bg_primary.getClassAttr)

  def bgPrimarySubtle: BSProgress =
    copy(progressBarClasses = progressBarClasses + bg_primary_subtle.getClassAttr)

  def bgSecondary: BSProgress = copy(progressBarClasses = progressBarClasses + bg_secondary.getClassAttr)

  def bgSecondarySubtle: BSProgress =
    copy(progressBarClasses = progressBarClasses + bg_secondary_subtle.getClassAttr)

  def bgSuccess: BSProgress = copy(progressBarClasses = progressBarClasses + bg_success.getClassAttr)

  def bgSuccessSubtle: BSProgress =
    copy(progressBarClasses = progressBarClasses + bg_success_subtle.getClassAttr)
  ...
  def bgWarningSubtle: BSProgress =
    copy(progressBarClasses = progressBarClasses + bg_warning_subtle.getClassAttr)

  def bgDanger: BSProgress = copy(progressBarClasses = progressBarClasses + bg_danger.getClassAttr)

  def bgDangerSubtle: BSProgress =
    copy(progressBarClasses = progressBarClasses + bg_danger_subtle.getClassAttr)
  ...
  def render(percent: Double, label: String = ""): Elem =
    <div class={progressClasses} role="progressbar" aria-valuenow={
      percent.formatted("%.2f")
