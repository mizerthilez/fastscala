...
import com.fastscala.components.bootstrap5.utils.ImmediateInputFields
...
class DateFieldOpt(
...
)(implicit renderer: DateFieldOptRenderer
  ...
  override def onEvent(
    ...
    case PerformSave =>
      set(for year <- currentYear; month <- currentMonth; day <- currentDay
      yield new LocalDate(year, month, day))
    case _ => JS.void
  )

  override def hasErrors_?(): Boolean =
    required() && (currentYear.isEmpty || currentMonth.isEmpty || currentDay.isEmpty)
  ...
  def render()(implicit form: Form5, fsc: FSContext, hints: Seq[RenderHint]): Elem =
    withFieldRenderHints { implicit hints =>
      val dayField = JS.rerenderable(rerenderer =>
        implicit fsc =>
          def maxDays: Int = (for
            year <- currentYear
            month <- currentMonth
      ...
      val monthField = ImmediateInputFields.select[Option[Int]](
        ...
        value =>
          currentMonth = value
          dayField.rerender() & form
            .onEvent(ChangedField(this)) & (if hints.contains(ShowValidationsHint) then reRender()
                                            else JS.void)
        ,
        toString = idx =>
          idx
