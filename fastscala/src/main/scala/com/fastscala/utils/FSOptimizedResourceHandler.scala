...
object FSOptimizedResourceHandler:
  def cssLoaderUrl(files: String*): String = "/static/optimized/css_loader.css?" + files
    .map("f=" + URLEncoder.encode(_, "UTF-8"))
    .mkString("&")
...
class FSOptimizedResourceHandler(
  ...
  override def handlerNoSession(
    ...
    case Get("static", "optimized", "css_loader.css") =>
      val files = Option(Request.getParameters(req).getValues("f"))
        .getOrElse(Collections.emptyList)
        .asScala
        .reverse
      val css: String = cssLoaderCache.getOrElseUpdate(
        files.mkString(";"),
        files.toList
    ...
    case r @ Req(Get, "static" :: "optimized" :: restOfPath, suf @ ("jpg" | "jpeg"), _) =>
      ...
      imgCache.getOrElseUpdate(
        r.getHttpURI.getPath, {
          val remaining =
            java.net.URLDecoder.decode(restOfPath.mkString("", "/", "." + suf), "UTF-8")
          ...
          then
            ...
            val compression: Option[Int] =
              Option(parms.getValue("compression")).flatMap(_.toIntOption)
            val original: Boolean =
              Option(parms.getValue("original")).flatMap(_.toBooleanOption).getOrElse(false)
            val is = resource.openStream()
            try
              val byteArray: Array[Byte] =
                if original then IO.readBytes(is)
                else
