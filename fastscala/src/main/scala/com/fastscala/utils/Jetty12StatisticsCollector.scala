...
class Jetty12StatisticsCollector(val statisticsHandler: StatisticsHandler) extends MultiCollector:
  override def collect: MetricSnapshots =
    ...
    Seq(
      ...
      buildCounter(
        "jetty_handler_failed_total",
        "Number of requests that throw an exception from handle()",
        statisticsHandler.getHandlingFailures,
      ),
      buildGauge(
        "jetty_handler_active",
        "Number of requests currently in handle()",
        statisticsHandler.getHandleActive,
      ),
      buildGauge(
        "jetty_handler_active_max",
        "Maximum number of requests in handle()",
