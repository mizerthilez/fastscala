...
class RerendererP[Env <: FSXmlEnv, P]
  ...
  def rerender(param: P): Js = Js.replace(aroundId, render(param)(rootRenderContext.getOrElse(throw new Exception("Missing context - did you call render() first?")))) // & Js(s"""$$("#$aroundId").fadeOut(100).fadeIn(100).fadeOut(100).fadeIn(100)""")
  ...
  def map(f: env.Elem => env.Elem): RerendererP[env.type, P] = {
    new RerendererP[env.type, P](null, None, None) {
      ...
      override def rerender(param: P): Js = Js.replace(self.aroundId,
        f(self.render(param)(self.rootRenderContext.getOrElse(throw new Exception("Missing context - did you call render() first?"))))) // & Js(s"""$$("#$aroundId").fadeOut(100).fadeIn(100).fadeOut(100).fadeIn(100)""")
