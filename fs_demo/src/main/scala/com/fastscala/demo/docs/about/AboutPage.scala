...
class AboutPage extends MultipleCodeExamples2Page:
  ...
  override def renderContentsWithSnippets()(implicit fsc: FSContext): Unit =
      ...
      alert.alert_success
        ...
        .apply:
          div.apply(
            ...
            BSBtn().BtnPrimary
              .lbl("Register for Free Training!")
    ...
    renderSnippet("Create a callback"):
      val callbackJs =
        fsc.callback(() => JS.alert(s"Current date/time on server: ${new Date().toString}"))
      <p>Clicking the button runs the javacript:</p>
      <pre>{callbackJs.cmd}</pre>
      <button class="btn btn-primary d-block mx-auto" onclick={
        callbackJs.cmd
    ...
    renderSnippet("Check time on server basic example"):
      <div id="current-time"><span>{new Date().toString}</span></div> ++
        BSBtn().BtnPrimary.sm
          .lbl("Update time")
          .ajax(_ => JS.setContents("current-time", <span>{new Date().toString}</span>))
          .sm
          .btn
    renderHtml():
      <p>
        ...
        implicit fsc =>
          queryOpt =>
            queryOpt match
              case Some(query) =>
                val url = new URL(
                  s"https://api.dictionaryapi.dev/api/v2/entries/en/${URLEncoder.encode(query)}"
                )
                ...
                try
                  Try(Source.fromInputStream(con.getInputStream).mkString).toEither.flatMap{ json =>
                    ...
                    case Left(_: java.io.FileNotFoundException) =>
                      span.text_danger.apply("No results found.")
