...
class RerenderablePage extends MultipleCodeExamples2Page():
  ...
  override def renderContentsWithSnippets()(implicit fsc: FSContext): Unit =
    ...
    renderSnippet("Rerenderable from outside"):
      val rerenderer = JS.rerenderableContents(rerenderer =>
        implicit fsc =>
          div
            .withStyle(
              s"height: 100px; width: 300px;background-color: rgb(${math.random() * 80},${math.random() * 80},${math.random() * 80})"
            )
            .text_white
            .rounded_1
            .rounded
            ...
            .p_2
            .my_2
            .apply(s"Time on server: ${new Date().toString}")
      )
      rerenderer.render() ++
    ...
    renderSnippet("Rerenderable from inside the block"):
      JS.rerenderableContents(rerenderer =>
        implicit fsc =>
          div
            .withStyle(
              s"height: 100px;background-color: rgb(${math.random() * 80},${math.random() * 80},${math.random() * 80})"
            )
            .text_white
            .rounded_1
            .rounded
    ...
    renderSnippet("Rerenderable with arguments"):
      val rerenderer = JS.rerenderableContentsP[Option[Int]](rerenderer =>
        implicit fsc =>
          pointsOpt =>
            div.text_center.fs_1.fw_bolder
              .withStyle(
                s"height: 100px;background-color: rgb(${math.random() * 80},${math.random() * 80},${math.random() * 80})"
              )
              .text_white
