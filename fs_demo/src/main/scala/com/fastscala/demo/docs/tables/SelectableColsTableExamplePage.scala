...
class SelectableColsTableExamplePage extends MultipleCodeExamples2Page:
  def pageTitle: String = "Selectable cols table example"
  ...
  def renderContentsWithSnippets()(implicit fsc: FSContext): Unit =
    renderSnippet("Source"):
      ...
      lazy val mainTable = new Table5Base
        ...
        override def columnStartsVisible(c: Table5StandardColumn[Country]): Boolean =
          Set(
            ColName,
            ColUNMember,
            ColCapital,
      ...
      new Widget:
        ...
        override def widgetTopRight()(implicit fsc: FSContext): NodeSeq =
          mainTable.colSelectionDropdownBtn(_.BtnPrimary.lbl("Columns...").sm.dataBsAutoCloseAsOutside)
