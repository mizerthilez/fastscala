...
class InternalMetricsPage extends MultipleCodeExamples2Page:
  def pageTitle: String = "Internal Metrics"
  ...
  def renderContentsWithSnippets()(implicit fsc: FSContext): Unit =
    renderSnippet("Source"):
      ...
      val rerenderable = JS.rerenderable(rerenderer =>
        implicit fsc =>
          ...
          div.apply {
            <div> {
              BSProgress().striped.animated.bgWarning
                .renderXofY(usedMem, totalMem, readableSize(usedMem))
                .mt_1
                .mb_2
            } </div>
            <ul>
              ...
              <li class="ms-2"><b>Total #Defunct Pages:</b> {
              fsc.session.fsSystem.sessions.map(_._2.pages.count(_._2.isDefunct_?)).sum
            }</li>
              <li><b>Total #Anonymous Pages:</b> {
              fsc.session.fsSystem.sessions.map(_._2.anonymousPages.size).sum
            }</li>
              <li><b>Total #Callback Functions:</b> {
              fsc.session.fsSystem.sessions.flatMap(_._2.pages.map(_._2.callbacks.size)).sum
            }</li>
              <li><b>Total #File Download Callback Functions:</b> {
              fsc.session.fsSystem.sessions.flatMap(_._2.pages.map(_._2.fileDownloadCallbacks.size)).sum
            }</li>
              <li><b>Total #File Upload Callback Functions:</b> {
              fsc.session.fsSystem.sessions.flatMap(_._2.pages.map(_._2.fileUploadCallbacks.size)).sum
            }</li>
            </ul>
            <h6>This session:</h6>
            <ul>
              ...
              <li><b>#File Download Callback Functions:</b> {
              fsc.session.pages.map(_._2.fileDownloadCallbacks.size).sum
            }</li>
              <li><b>#File Upload Callback Functions:</b> {
              fsc.session.pages.map(_._2.fileUploadCallbacks.size).sum
            }</li>
            </ul>
          }
      )
      ...
      def updateMetrics(): IO[Unit] = for
        ...
        _ <- if fsc.page.isDefunct_? then IO.unit else updateMetrics()
      yield ()
