...
object ClickToEditFields:
  ...
  def Str(
    ...
    def editing: NodeSeq =
      ...
      transformEditElem(get())(<input class={editElemClasses} id={elemId} value={get()} onblur={submit} type="text" onkeypress={JS.onkeypress(13, 27)(JS.blur(elemId)).cmd} placeholder={if placeholder == "" then null else placeholder}/>)
  ...
  def Textarea(
    ...
    def displaying: NodeSeq =
      ...
      <div id={aroundId}><span tabindex={tabindex + ""} class={if get() == "" then emptyDisplayClasses else nonEmptyDisplayClasses} onclick={toEditJs} onkeypress={s"event = event || window.event; if ((event.keyCode ? event.keyCode : event.which) == 13) {$toEditJs}"}>{Some(get()).filter(_ != "").getOrElse(emptyString)}</span></div>
