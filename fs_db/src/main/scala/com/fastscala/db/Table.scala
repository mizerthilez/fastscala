...
trait Table[R] extends TableBase:
  ...
  def insertSQL(row: R): SQL[Nothing, NoExtractor] =
    val columns: SQLSyntax = SQLSyntax.createUnsafely(
      insertFields
        .map: field =>
          field.setAccessible(true)
          fieldName(field)
  ...
  def updateSQL(row: R, where: SQLSyntax = SQLSyntax.empty): SQL[Nothing, NoExtractor] =
    val values: SQLSyntax = updateFields
      .map: field =>
        field.setAccessible(true)
        sqls""""${SQLSyntax.createUnsafely(fieldName(field))}" = """ + valueToFragment(
          field,
          field.get(row),
        )
      .reduceOption(_ + sqls", " + _)
      .getOrElse(SQLSyntax.empty)

    sql"""update "$tableNameSQLSyntax" set $values $where"""

  def deleteSQL(row: R, where: SQLSyntax = SQLSyntax.empty): SQL[Nothing, NoExtractor] =
    sql"""delete from "$tableNameSQLSyntax" $where"""
