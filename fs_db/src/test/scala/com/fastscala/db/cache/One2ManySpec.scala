...
class Teacher(
  val name: String
) extends RowWithLongId[Teacher]
       with ObservableRow[java.lang.Long, Teacher]:
...
class Class(
  val name: String,
  var teacherId: Long,
) extends RowWithLongId[Class]
       with ObservableRow[java.lang.Long, Class]:
...
class One2ManySpec extends AnyFlatSpec with PostgresDB:
  class DBCache extends DBCompositeObserver:
    ...
    val class2Teacher = new One2ManyCache[java.lang.Long, Teacher, Class](
      teacher,
      clas,
      _.teacherId,
      id => sqls"where teacher_id = $id",
    )
